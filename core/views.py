...
def MakePredict(request):
	...
	clf = joblib.load('model/naive_model.pkl')
...
def patient_ment(request):
	user_id = request.user.id
	appointmnet = Ment.objects.all().filter(patient_id=user_id)
	context = {'ment':appointmnet, 'status':'1'}
...
def MakeMend(request):
  ...
  if sick in disease_list:
      ...
      clf = joblib.load('model/Medicine.pkl')
      prediction = clf.predict(test)
